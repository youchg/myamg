                                ...
                                amg_param param)
{
    ...
    double **dvec_amg = (double**)malloc(multistep*nev * sizeof(double*));
    for(i=0; i<multistep*nev; i++) dvec_amg[i] = (double*)calloc(A->nr, sizeof(double));
	...
	matP = amg->P[i];
		...
		if(m == 0)
		{
		    if(init_approx_level < 0)
		    {
			Multi_dmatcsr_dvec(matP, evec[j], dvec_amg[step*nev + j]);
		    }
		    else
		    {
			if(i==amg_correction_begin_level && is_init_approx_level_correction>0)
			    memcpy(dvec_amg[step*nev + j], evec[j], matA->nr*sizeof(double));
			else
			    Multi_dmatcsr_dvec(matP, evec[j], dvec_amg[step*nev + j]);
		    }
		}
	    ...
	    linear_time += Correction_solve_linear(amg, i, nev, eval, dvec_amg+step*nev, param);

	    int length = (m >= multistep-1) ? multistep : step+1;
...
#if   amg_eigen_expand_type == 1
	    expand_time += Correction_expand_matrix_RAhV_VTAhV_multistep(Alarge, matA, AH, amg, i, coarsest_level, nev, dvec_amg, length); 
	    expand_time += Correction_expand_matrix_RAhV_VTAhV_multistep(Mlarge, matM, MH, amg, i, coarsest_level, nev, dvec_amg, length); 
#elif amg_eigen_expand_type == 2
	    expand_time += Correction_expand_matrix_RAhV_VTRTAHRV(Alarge, matA, AH, amg, i, coarsest_level, nev, dvec_amg[step]); 
	    expand_time += Correction_expand_matrix_RAhV_VTRTAHRV(Mlarge, matM, MH, amg, i, coarsest_level, nev, dvec_amg[step]); 
#elif amg_eigen_expand_type == 3
	    expand_time += Correction_expand_matrix_AHRV_VTAhV(Alarge, matA, AH, amg, i, coarsest_level, nev, dvec_amg[step]); 
	    expand_time += Correction_expand_matrix_AHRV_VTAhV(Mlarge, matM, MH, amg, i, coarsest_level, nev, dvec_amg[step]); 
#elif amg_eigen_expand_type == 4
	    expand_time += Correction_expand_matrix_AHRV_VTRTAHRV(Alarge, matA, AH, amg, i, coarsest_level, nev, dvec_amg[step]); 
	    expand_time += Correction_expand_matrix_AHRV_VTRTAHRV(Mlarge, matM, MH, amg, i, coarsest_level, nev, dvec_amg[step]); 
#endif
	    ...
	    new_evec_time += Correction_get_new_evec_multistep(amg, i, coarsest_level, nev, dvec_amg, length, evec_expand, evec);
