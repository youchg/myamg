...
int main(int argc, char* argv[])
{
    ...
    Init_amg_param_argv(argc, argv, &param, &AA, &MM, NULL);
    Print_dmatcsr(AA);
    Print_dmatcsr(MM);

    dmatcsr *A = Sum_dmatcsr_mApnB(1.0, AA, shift, MM);
    ...
    for(i=0; i<size; i++) sol[i] = ((double)rand())/((double)RAND_MAX);
    ...
    int    cg_print_level = 1;
    Linear_solver_cg(A, b, x_cg, tol_cg, max_iter_cg, NULL, &resi_norm_cg, &niter_cg, cg_print_level);
    ...
    param_myamg.amgsolver_tol = 1e-14;
    param_myamg.amgcycle_print_level = 0;
    param_myamg.amgsolver_print_level = 9;

    double tb_setup = Get_time();
    multigrid *amg = Build_amg(AA, MM, param_myamg.max_level);
    ...
    Linear_solver_shift_amg(amg, shift, 0, b, x_amg, param_myamg, &resi_norm_amg, &ncycle_amg);
    ...
    param_pcg.pcg_amg_print_level = 1;
